...
> R: $2800. Cálculo: Leche ($350 * 2 = $700) + Pan ($200 * 3 = $600) + Queso ($1500 * 1 = $1500) = $700 + $600 + $1500 = $2800.

**P2** — ¿El total que imprime el programa coincide con lo que calculaste? Si no coincide, ¿cuánto muestra?

> R: No coincide. Muestra 2050 porque suma únicamente el precio unitario de cada producto sin multiplicarlo por su cantidad.
...
> R: Inicializa el carrito asignando `c->cantidad = 0`. Es fundamental llamarla antes de usarlo porque en C las variables locales ubicadas en la pila (stack) no se inicializan por defecto y contienen valores basura (indeterminados). Sin esta inicialización, el campo `cantidad` tendría un valor arbitrario, ocasionando fallos inmediatos y comportamiento indefinido.
...
> R: Pasa, porque al tener `cantidad = 1`, el precio unitario coincide numéricamente con el subtotal (350 * 1 = 350).
...
> R: Falla. Esperaba 700 ($350 * 2) y obtuvo 350, ya que la función `carrito_total` no multiplicaba el precio por la cantidad.
...
> R: Está en la función `carrito_total` (en la línea 24 del archivo original `src/carrito.c`), donde dice:
> `total += c->items[i].precio;`

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Debería multiplicar el precio unitario de cada producto por su cantidad:
...
> R: Falla. El 5to `carrito_agregar` devuelve 1 (éxito) en lugar de 0 (fallo), permitiendo agregar un quinto elemento e indexando fuera del límite del arreglo `MAX_ITEMS = 4`.
...
> R: El operador incorrecto es `<=` en `c->cantidad <= MAX_ITEMS`. Debería ser `<` (`c->cantidad < MAX_ITEMS`), ya que para un tamaño máximo de 4 los índices válidos son 0, 1, 2 y 3.
...
> R: Sí, las líneas 29 y 30 (la función `carrito_descuento`) tienen `#####`. No se ejecutaron porque la suite de `test_unitarios.c` no llama a `carrito_descuento` (esta función se evalúa en los tests de integración).
...
> R: Un test unitario prueba una función o componente individual de forma aislada e independiente, mientras que un test de integración verifica el comportamiento combinado y la interacción correcta entre múltiples funciones o módulos. El bug de `carrito_total` fue detectado primero por el test unitario (`test_total_con_cantidad`).
...
> R: En C no existe verificación automática de límites en tiempo de ejecución (bounds checking); escribir fuera de un array corrompe posiciones contiguas de memoria (variables, punteros o direcciones de retorno), provocando comportamientos erráticos, fallas de segmentación o graves vulnerabilidades de seguridad. En cambio, lenguajes como Python o Java verifican los límites en runtime y detienen la ejecución lanzando una excepción controlada (`IndexError` / `ArrayIndexOutOfBoundsException`), imposibilitando el acceso o corrupción indebida de memoria.
...
> R: Escribir tests proporciona una verificación objetiva, sistemática y reproducible de los requerimientos, cubriendo casos de borde que la inspección visual podría pasar por alto debido a sesgos cognitivos. Además, los tests quedan como una suite de regresión automatizada que protege el código contra futuros errores al refactorizar o agregar funcionalidades.
...
> R: Porque cuando `cantidad == 1`, el resultado de `precio * 1` es exactamente igual a `precio`. Por lo tanto, el cálculo erróneo (`total += precio`) coincidió casualmente con el valor esperado (350), enmascarando el defecto. Al probar con `cantidad == 2`, la diferencia entre `precio` (350) y `precio * 2` (700) hizo evidente el error.
